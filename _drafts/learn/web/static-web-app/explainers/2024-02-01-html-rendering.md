...
---
If you're not an web developer, the concepts of web applications, web services, and websites can be a little squishy. You may have also heard about web APIs and wondered what those are exactly and how they're different from a website.<!--more--> Let's try to make sense of all this.
...
PHP is a language that works to generate dynamic web content by a web server. Let's say the *my-balance.html* page of your bank is generated by the following PHP.
...
In this example, the *customer-data.php* file contains all the detailed code that looks up customer information. The actual HTML that is generated and sent to the visitor's browser would be:
...
JavaScript is code that is part of the HTML and runs on the web browser during or after the HTML is loaded.
...
The HTML contains the `script` tag at the bottom which writes the current time in the body of the HTML when the page is loaded. Running code in the browser is known as **client-side rendering**. Your web browser is known as the **client**, and I'll start using that term for the rest of this post.
...
I want to highlight that this is *not* the same as dynamic content or server-side rendering. The web server is not generating dynamic HTML on the fly. The static site generator creates static HTML and then the web server hosts those static files.
