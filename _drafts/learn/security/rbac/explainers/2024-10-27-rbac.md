...
The scope means at what level in the [Azure hierarchy]({% post_url /learn/basics/azure_intro/explainers/2024-01-12-azure-hierarchy %}#) the permission will apply. Let's say you assign the Storage Blob Data Contributor role to the user kyle@{{ site.fake_company_domain }} at the subscription scope. That user will be able to read and write any blobs in the subscription. If you instead apply that at the level of a resource group named `KylesStuff`, kyle@{{ site.fake_company_domain }} will only be able to read and write blobs in that resource group. And finally, you may decide to apply it directly to the Storage account named `kylesweirdfilesdontlook`, then kyle@{{ site.fake_company_domain }} will only be able to read and write blobs in that Storage account.
