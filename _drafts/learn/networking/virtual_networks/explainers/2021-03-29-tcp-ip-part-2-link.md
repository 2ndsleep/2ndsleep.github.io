...
We'll discuss this more in the next post, but all of the computers in our example that are connected to the switch are part of a *broadcast domain*. You can think of a broadcast domain as a network space where all the computers can directly talk to each other without going through a router. Right now, we've only been looking at switches but when frames hit a router, things work a little differently. Even if you connected a bunch of switches together, all of the computers connected to those switches would be on the same broadcast domain.

Technically, a broadcast domain is the network space where *Ethernet braodcast frames* will be received. A broadcast frame is a special Ethernet frame that switches know to pass along to all the connected devices and devices know to listen to them instead of discarding them. In the example below, if any computer sent a broadcast frame, the switches would forward that frame to all of the other three computers, and all the computers would receive and process the broadcast frame.
