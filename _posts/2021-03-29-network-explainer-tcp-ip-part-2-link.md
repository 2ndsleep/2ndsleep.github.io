...
Technically, a broadcast domain is the network space where *Ethernet braodcast frames* will be received. A broadcast frame is a special Ethernet frame that switches know to pass along to all the connected devices and devices know to listen to them instead of discarding them. In our example above, if any computer send a broadcast frame, the switch would forward that frame to all computers on the switch and all the computers would receive and process the broadcast frame.